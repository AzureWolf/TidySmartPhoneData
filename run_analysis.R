...
getCleanTable <- function(groupName) {
	...
	### Step 2's function is also called here
...
### Remove data folder; no longer needed. For re-running purposes
...
getMeany <- function(combinedGroup) {
	...
	colNumbers <- ncol(combinedGroup);
	
	for(subject in unique(combinedGroup$Subject)) {
		values <- data.frame()
		for(activity2 in unique(combinedGroup$activity)) {
			values <- rbind(colMeans(subset(combinedGroup, Subject == subject & activity == activity2)[,3:colNumbers]))
	...
	sortedMeans <- sortedMeans[with(sortedMeans, order(as.numeric(as.character(Subject)), activity)),];
	sortedMeans;
}

tidySubjectActivity <- getMeany(combinedGroup);
write.table(tidySubjectActivity, file = "tidySubjectActivity.txt", row.names = FALSE)